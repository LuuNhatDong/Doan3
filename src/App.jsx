...
const buildAvatarUrl = (user) => {
  ...
  if (user.avatar && user.avatar.startsWith('/')) {
    return `https://doan3-ooha.onrender.com${user.avatar}`;
  }
...
function App() {
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([
      fetch('https://doan3-ooha.onrender.com/api/events').then(r => r.json()).catch(() => []),
      fetch('https://doan3-ooha.onrender.com/api/users').then(r => r.json()).catch(() => []),
      fetch('https://doan3-ooha.onrender.com/api/dashboard/pending-proofs').then(r => r.json()).catch(() => []),
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      const res = await fetch('https://doan3-ooha.onrender.com/api/profile', {
        method: 'PUT',
        body: formData,
      });
  ...
  const handleSaveProfile = () => {
    ...
    fetch('https://doan3-ooha.onrender.com/api/profile', {
      method: 'PUT',
      body: formData,
    })
      .then(async (res) => {
