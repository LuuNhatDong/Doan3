...
const EventManagement = ({ searchQuery = '' }) => {
  ...
  const [sampleProofFile, setSampleProofFile] = useState(null);
  ...
  const fetchFaculties = () => {
    fetch('https://doan3-ooha.onrender.com/api/faculties')
      .then(res => res.json())
      .then(data => {
  ...
  const fetchCriteria = () => {
    fetch('https://doan3-ooha.onrender.com/api/criteria')
      .then(res => res.json())
      .then(response => {
  ...
  const fetchEvents = () => {
    fetch('https://doan3-ooha.onrender.com/api/events')
      .then(res => res.json())
      .then(data => {
        const now = new Date();
        ...
        data.forEach(evt => {
          if (evt.status !== 'Ngừng hoạt động') {
            ...
            if (targetStatus !== evt.status) {
              fetch(`https://doan3-ooha.onrender.com/api/events/${evt.id}/status`, {
                method: 'PATCH',
  ...
  const handleOpenCreate = () => {
    ...
    setPosterFile(null); setSampleProofFile(null); setAttachedFiles([]);
  ...
  const handleOpenManage = (evt) => {
    ...
    fetch(`https://doan3-ooha.onrender.com/api/events/${evt.id}/participants`)
      .then(res => {
  ...
  const fetchLocationPresets = () => {
    return fetch('https://doan3-ooha.onrender.com/api/location-presets')
      .then(res => res.json())
      .then(data => {
  ...
  const saveLocationPreset = async (name, coords) => {
    ...
    try {
      const response = await fetch('https://doan3-ooha.onrender.com/api/location-presets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const removeLocationPreset = async (presetId) => {
    ...
    try {
      const response = await fetch(`https://doan3-ooha.onrender.com/api/location-presets/${presetId}`, { method: 'DELETE' });
      const data = await response.json();
  ...
  const handleSubmit = async (e, isEditMode) => {
    ...
    payload.append('score_type', formData.score_type); 
    ...
    if (sampleProofFile) payload.append('sample_proof', sampleProofFile);
    ...
    const apiUrl = isEditMode ? `https://doan3-ooha.onrender.com/api/events/${eventId}` : 'https://doan3-ooha.onrender.com/api/events';
  ...
  const handleDeactivate = (id, currentName) => {
    if (window.confirm(`⚠️ Ngừng hoạt động sự kiện [${currentName}]?`)) {
      fetch(`https://doan3-ooha.onrender.com/api/events/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'Ngừng hoạt động' }) }).then(() => fetchEvents());
    }
  };
  ...
  const applyFilters = () => {
    ...
    if (localSearchQuery) {
      filtered = filtered.filter(event =>
        (event.name && event.name.toLowerCase().includes(localSearchQuery.toLowerCase())) ||
        (event.id && event.id.toString().toLowerCase().includes(localSearchQuery.toLowerCase()))
      );
    }
          ...
          <div className="bg-white p-4 rounded-3 border shadow-sm">
            ...
            <Form.Group className="mb-3">
              ...
              <div className="border border-1 rounded p-3 text-center position-relative bg-light" style={{ borderStyle: 'dashed !important', borderColor: '#c4c4c4' }}>
                ...
                <div className="fw-medium small mt-2">{posterFile ? posterFile.name : "Chọn tệp hoặc kéo thả"}</div>
            ...
            <Form.Group className="mb-3">
              <Form.Label className="fw-semibold small text-dark">Tải ảnh minh chứng mẫu (Đối chiếu AI)</Form.Label>
              <div className="border border-1 rounded p-3 text-center position-relative bg-light" style={{ borderStyle: 'dashed !important', borderColor: '#c4c4c4' }}>
                <i className="bi bi-shield-check fs-3 text-success"></i>
                <div className="fw-medium small mt-2">{sampleProofFile ? sampleProofFile.name : "Chọn tệp mẫu hoặc kéo thả"}</div>
                <div className="text-muted" style={{ fontSize: '11px' }}>JPG, PNG lên đến 5MB (Mẫu chứng chỉ/kết quả mẫu)</div>
                <Form.Control type="file" accept="image/*" className="position-absolute top-0 start-0 w-100 h-100 opacity-0" style={{ cursor: 'pointer' }} onChange={(e) => setSampleProofFile(e.target.files[0])} />
              </div>
      ...
      <div className="bg-white p-4 rounded-3 border mb-4 shadow-sm">
        <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          ...
          <div className="d-flex align-items-center bg-light px-3 py-2 rounded border flex-wrap gap-2">
            ...
            <div className="d-flex align-items-center">
              <label htmlFor="proof-switch" className="fw-semibold small me-2 text-danger mb-0" style={{ cursor: 'pointer' }}>Bắt buộc nộp ảnh</label>
...
<div className="border rounded-3 shadow-sm bg-white overflow-hidden">
  <Table hover responsive size="sm" className="mb-0 align-middle">
    ...
    <tbody style={{ fontSize: '0.9rem' }}>
      {loadingParticipants ? (
        <tr><td colSpan="9" className="text-center py-4"><Spinner animation="border" size="sm" /> Đang tải danh sách...</td></tr>
      ) : participants.length === 0 ? (
        <tr><td colSpan="9" className="text-center py-4 text-muted">Chưa có sinh viên nào điểm danh.</td></tr>
      ) : (
        participants.map((p, idx) => (
          <tr key={idx}>
            ...
            <td className="py-2 text-center">
              ...
              {p.link && (
                <a href={p.link} target="_blank" rel="noreferrer" className="btn btn-sm btn-outline-success py-0 px-2 mb-1" title="Mở Link liên kết">
                  <i className="bi bi-link-45deg"></i> Link
                </a>
              )}
              {!p.file && !p.link && <span className="text-muted small">Chưa nộp</span>}
            </td>
          </tr>
        ))
      )}
