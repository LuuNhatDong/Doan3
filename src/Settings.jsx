...
// --- HÀM TẢI DỮ LIỆU TỪ SERVER BACKEND ---
  const fetchCriteria = async () => {
    ...
    try {
      ...
      // BỔ SUNG: Gọi API lấy thông số cấu hình Trợ lý AI đang lưu trên Server
      const aiRes = await fetch('https://doan3-ooha.onrender.com/api/system-settings');
      const aiJson = await aiRes.json();
      if (aiJson.status === 'success') {
        setAutoApprove(aiJson.data.autoApprove);
        setOcrThreshold(aiJson.data.ocrThreshold);
        setHammingDistance(aiJson.data.hammingDistance);
        setContextPoints(aiJson.data.contextPoints);
      }
    } catch (error) {
      console.error("Lỗi kết nối đồng bộ hệ thống:", error);
    } finally {
      setLoading(false);
    }
  };
// 1. Thêm một useEffect mới để tự động gọi tải cấu hình hệ thống khi vừa mở trang Settings
useEffect(() => {
  const fetchAISettings = async () => {
    try {
      const res = await fetch('https://doan3-ooha.onrender.com/api/system-settings');
      const json = await res.json();
      if (json.status === 'success') {
        setAutoApprove(json.data.autoApprove);
        setOcrThreshold(json.data.ocrThreshold);
        setHammingDistance(json.data.hammingDistance);
        setContextPoints(json.data.contextPoints);
      }
    } catch (error) {
      print('⚠️ Không thể tải cấu hình AI từ máy chủ:', error);
    }
  };
  fetchAISettings();
}, []);

// BỔ SUNG: Hàm xử lý đẩy thông số thanh trượt lưu vào cơ sở dữ liệu backend
  const handleSaveAISettings = async () => {
    try {
      ...
      if (data.status === 'success') {
        ...
        alert("⚠️ Không thể lưu cấu hình: " + data.message);
      }
    } catch (error) {
      alert("❌ Lỗi kết nối: Không thể gửi dữ liệu cấu hình tới Server Node.js!");
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modalMode === 'add_main' || modalMode === 'edit_main') {
      url = 'https://doan3-ooha.onrender.com/api/criteria/main';
      ...
      url = 'https://doan3-ooha.onrender.com/api/criteria/sub';
