                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Hiển thị phản hồi từ chối của cán bộ quản lý (Admin Comment) nếu có
...
IconButton(
  ...
  onPressed: () async {
    // Xử lý làm sạch chuỗi URL tránh lỗi dư dấu gạch chéo
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Đang mở tài liệu: $fileName...'))
    );

    // Gọi trình duyệt ngoài để tải file
    if (await canLaunchUrl(url)) {
      await launchUrl(url, mode: LaunchMode.externalApplication);
    } else {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('❌ Không thể mở đường dẫn tệp!'), backgroundColor: Colors.red)
        );
      }
    }
  },
