    ...
    try {
      ...
      if (!isRegistered) {
        ...
        try {
          ...
          _showErrorAndResume('Lỗi mạng khi đăng ký: $e');
          return;
        }
      }

      bool requireProof = matchedEvent['require_proof'] == 1 || matchedEvent['require_proof'] == '1' || matchedEvent['require_proof'] == true;
      bool requireGps = matchedEvent['require_gps'] == 1 || matchedEvent['require_gps'] == '1' || matchedEvent['require_gps'] == true;

      // KIỂM TRA BÁN KÍNH GPS
      if (requireGps) {
        double eventLat = double.tryParse(matchedEvent['latitude']?.toString() ?? '0') ?? 0.0;
        double eventLng = double.tryParse(matchedEvent['longitude']?.toString() ?? '0') ?? 0.0;

        if (eventLat != 0.0 && eventLng != 0.0 && _currentPosition != null) {
          ...
          if (distanceInMeters > 50) {
            _showErrorAndResume('Bạn đang ở quá xa vị trí sự kiện (${distanceInMeters.toStringAsFixed(0)} mét). Vui lòng đến đúng địa điểm!');
            return; 
          }
        }
      }
      ...
      if (!requireProof) {
        ...
        try {
          ...
          if (checkInRes.data['status'] == 'success') {
            ...
            _showErrorAndResume('Điểm danh thất bại: ${checkInRes.data['message']}');
          }
        } catch (e) {
